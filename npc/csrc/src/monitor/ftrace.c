...
//找到Section header
    ...
    shdr = (Elf32_Shdr*)malloc(ehdr.e_shnum * ehdr.e_shentsize);//所有的Section header
    if(!shdr){
        printf("error malloc\n");
        fclose(fp);
        return;
...
//找到Section header的节区头表
    ...
    if((shstrtab = (char *)malloc(shstrtab_shdr->sh_size))==NULL){//获取内容
...
//遍历寻找.symtab .strtab
    for(int i=0;i< ehdr.e_shnum;i++){
       // printf("shstrtab : %s  shdr[i].sh_name %d\n",shstrtab,shdr[i].sh_name);
        ...
        if(strcmp(Section_name,".symtab")==0){
            if((symtab = (Elf32_Sym*)malloc(shdr[i].sh_size))==NULL){
           ...
           if ((strtab = (char *)malloc(shdr[i].sh_size)) == NULL) {
                perror("malloc");
                free(shstrtab);
                free(shdr);
    ...
    symbol_count =0;
    symbol_data =(symbol_str *) malloc(symtab_size *sizeof(symbol_str));
    if(!symbol_data){
        printf("error\n");
    }
    for(int i=0;i<symtab_size;i++){
        ...
        if (type == STT_FUNC && sym->st_name != 0) {  // st_name=0 表示无名符号
            ...
            symbol_data[symbol_count].name = (char *)malloc(strlen(name) + 1);
...
extern "C" void call_ftrace_printf(uint32_t pc,uint32_t dnpc)
{
    ++depth;
...
extern "C" void ret_ftrace_printf(uint32_t pc){
    --depth;
    for(int i=0;i<symbol_count;i++){
