...
#define immB() do { *imm = (SEXT(BITS(i,31,31),1)<<12)|(BITS(i,30,25)<<5)|(BITS(i,11,8)<<1)|(SEXT(BITS(i,7,7),1)<<11);} while(0)
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, s->dnpc = (src1 + imm)& ~(word_t)1;R(rd) = s->pc + 4;
    ...
    IFDEF(CONFIG_FTRACE,{
      if(s->isa.inst==0x00008067){
        IFDEF(CONFIG_FTRACE,ret_ftrace_printf(s->pc));
      }//区分jalr 和ret
      else if(rd ==1){
        IFDEF(CONFIG_FTRACE,call_ftrace_printf(s->pc,s->dnpc));
      }
    })
  );
  ...
  INSTPAT("??????? ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = src1 >> imm |SEXT(BITS(src1,31,31),1)<<(32-imm));
  ...
  INSTPAT("0000000 ????? ????? 010 ????? 01100 11", slt    , R, R(rd) = (int)src1 < (int)src2 ? 1 : 0);//有符号
  INSTPAT("0000000 ????? ????? 011 ????? 01100 11", sltu   , R, R(rd) = (uint32_t)src1 < (uint32_t)src2 ?1:0);//有符号
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, int32_t temp1 = src1;int32_t temp2 = src2;int64_t temp = (int64_t)temp1 * (int64_t)temp2;R(rd) = BITS(temp,63,32));
